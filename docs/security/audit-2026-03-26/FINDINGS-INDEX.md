...
| PA-025 | CLI JWT verify does not pin algorithm (algorithm confusion risk) | 5.9 | CWE-327 | A02 | `cliAuth.middleware.ts:38` | Open |
| PA-026 | Settings files with encrypted credentials written world-readable (0644) | 5.5 | CWE-732 | A01 | Multiple `settings.ts` files | Open |
| PA-027 | Trust proxy hop count hardcoded — IP spoofing for rate limiters | 5.3 | CWE-348 | A05 | `server.ts:53` | Open |
...
| PA-038 | .env.example contains copy-pasteable insecure SESSION_SECRET | 4.3 | CWE-1188 | A05 | `backend/.env.example:18` | Open |
...
| PA-046 | .gitignore missing *.pfx, *.key, *.pem, *.db patterns | 4.7 | CWE-312 | A05 | `.gitignore` | Open |
| PA-047 | Frontend .dockerignore does not exclude .env files | 4.7 | CWE-312 | A05 | `frontend/.dockerignore` | Open |
...
| PA-049 | Frontend Dockerfile runs nginx as root (no USER directive) | 3.7 | CWE-250 | A05 | `frontend/Dockerfile` | Open |
