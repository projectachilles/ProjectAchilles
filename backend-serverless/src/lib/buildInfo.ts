 ...
 *
 * Resolved from process.cwd(), not import.meta.url: @vercel/node bundles the
 * source with ncc/esbuild, so the directory layout at runtime does not match
 * the source tree and an `import.meta.url`-derived path points into bundle
 * internals. cwd is /var/task in the Vercel runtime.
 */
