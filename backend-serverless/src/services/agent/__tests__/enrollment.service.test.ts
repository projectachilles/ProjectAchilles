...
// Generate a deterministic Ed25519 key pair for tests and set env vars.
// The serverless signing service reads keys from env vars (not disk), so we
// must provide them before any enrollment code that uses encryption runs.
import crypto from 'crypto';
const { publicKey: _testPub, privateKey: _testPriv } = crypto.generateKeyPairSync('ed25519');
process.env.SIGNING_PRIVATE_KEY_B64 = _testPriv.export({ type: 'pkcs8', format: 'der' }).toString('base64');
process.env.SIGNING_PUBLIC_KEY_B64 = _testPub.export({ type: 'spki', format: 'der' }).toString('base64');

// Mock signing service — ensureSigningKeyPair is a no-op (env vars set above),
// override getPublicKeyBase64 for deterministic enrollment response.
...
describe('enrollment.service', () => {
  ...
  describe('createToken', () => {
    ...
    it('stores hashed token in database', async () => {
      ...
      const row = await testDb.get('SELECT * FROM enrollment_tokens WHERE id = ?', [result.id]) as unknown as any;
  ...
  describe('enrollAgent', () => {
    ...
    it('increments token use_count after enrollment', async () => {
      ...
      const row = await testDb.get('SELECT use_count FROM enrollment_tokens WHERE id = ?', [token.id]) as unknown as any;
    ...
    it('creates agent record in database', async () => {
      ...
      const agent = await testDb.get('SELECT * FROM agents WHERE id = ?', [result.agent_id]) as unknown as any;
  ...
  describe('listTokens', () => {
    it('returns active tokens for an org', async () => {
      ...
      const tokens = await listTokens('org-001');
    ...
    it('returns empty array for unknown org', async () => {
      const tokens = await listTokens('nonexistent');
  ...
  describe('rotateAgentKey', () => {
    ...
    it('stores pending hash, not primary (grace period model)', async () => {
      ...
      const oldRow = await testDb.get('SELECT api_key_hash FROM agents WHERE id = ?', [enrolled.agent_id]) as unknown as any;
      const oldHash = oldRow.api_key_hash;

      await rotateAgentKey(enrolled.agent_id);

      const row = await testDb.get('SELECT api_key_hash, pending_api_key_hash, key_rotation_initiated_at FROM agents WHERE id = ?', [enrolled.agent_id]) as unknown as any;
    ...
    it('stores encrypted pending key', async () => {
      ...
      const row = await testDb.get('SELECT pending_api_key_encrypted FROM agents WHERE id = ?', [enrolled.agent_id]) as unknown as any;
    ...
    it('does not set api_key_rotated_at until promotion', async () => {
      ...
      const agent = await testDb.get('SELECT api_key_rotated_at FROM agents WHERE id = ?', [enrolled.agent_id]) as unknown as any;
    ...
    it('new key validates against pending hash', async () => {
      ...
      const row = await testDb.get('SELECT api_key_hash, pending_api_key_hash FROM agents WHERE id = ?', [enrolled.agent_id]) as unknown as any;
    ...
    it('double rotation replaces previous pending key', async () => {
      ...
      const row1 = await testDb.get('SELECT pending_api_key_hash FROM agents WHERE id = ?', [enrolled.agent_id]) as unknown as any;

      const result2 = await rotateAgentKey(enrolled.agent_id);
      const row2 = await testDb.get('SELECT pending_api_key_hash FROM agents WHERE id = ?', [enrolled.agent_id]) as unknown as any;
    ...
    it('promotePendingKey moves pending to primary', async () => {
      ...
      await promotePendingKey(enrolled.agent_id);

      const row = await testDb.get('SELECT api_key_hash, pending_api_key_hash, pending_api_key_encrypted, key_rotation_initiated_at, api_key_rotated_at FROM agents WHERE id = ?', [enrolled.agent_id]) as unknown as any;
  ...
  describe('revokeToken', () => {
    it('deletes a token', async () => {
      ...
      const row = await testDb.get('SELECT * FROM enrollment_tokens WHERE id = ?', [token.id]);
      expect(row).toBeUndefined();
    });

    it('throws for nonexistent token', async () => {
      await expect(revokeToken('nonexistent')).rejects.toThrow('Token not found');
    });
  });
});
