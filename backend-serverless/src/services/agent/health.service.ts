...
// Computed on-demand from indexed tables (cheap, no caching needed).
// Async version for serverless backend (Turso / @libsql/client).
 ...
 */
export async function computeAgentHealthScore(agentId: string): Promise<number> {
  const db = await getDb();

  // Heartbeat consistency: heartbeat_history samples every 5th heartbeat.
  // Expected = 7 days x 24h x 60min / 5 = 2016 records.
  const hbRow = await db.get(
  ...
  const taskRow = await db.get(
    ...
    [agentId]
  ) as unknown as { completed: number | null; failed: number | null };
  ...
  const disconnectRow = await db.get(
 ...
 */
export async function computeAgentHealthScores(agentIds: string[]): Promise<Map<string, number>> {
  ...
  const taskRows = await db.all(
    `SELECT agent_id,
     ...
     GROUP BY agent_id`,
    [...agentIds]
  ) as unknown as { agent_id: string; completed: number | null; failed: number | null }[];
  ...
  const disconnectRows = await db.all(
