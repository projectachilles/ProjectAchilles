...
async function freshenNextRun(schedule: Schedule): Promise<Schedule> {
  ...
  if (resolveRandomizeMode(schedule.schedule_config) === 'per_machine' && schedule.agent_next_runs) {
    ...
    await db.run(
      'UPDATE schedules SET next_run_at = ?, agent_next_runs = ?, updated_at = datetime(\'now\') WHERE id = ?',
      [freshMin, JSON.stringify(map), schedule.id]
    );
...
/**
 * Resolve the effective randomization mode, bridging the legacy `randomize_time`
 * boolean to the `randomize_mode` enum. Explicit `randomize_mode` wins; otherwise
 * legacy `randomize_time:true` → 'fleet', anything else → 'fixed'.
 */
...
/**
 * Compute an independent next-run time for each agent (per_machine mode). Calls
 * `computeNextRun` once per agent — each call re-rolls the random time-of-day.
 * Returns a map of agentId → UTC ISO string.
 */
...
export async function createSchedule(
  ...
  const row = await db.get('SELECT * FROM schedules WHERE id = ?', [id]) as unknown as ScheduleRow;
...
export async function getSchedule(id: string): Promise<Schedule> {
  const db = await getDb();
  const row = await db.get('SELECT * FROM schedules WHERE id = ?', [id]) as unknown as ScheduleRow | undefined;
...
export async function updateSchedule(
  ...
  const existing = await db.get('SELECT * FROM schedules WHERE id = ?', [id]) as unknown as ScheduleRow | undefined;
  ...
  if (updates.schedule_config || updates.timezone || updates.status || updates.agent_ids) {
    const updated = await db.get('SELECT * FROM schedules WHERE id = ?', [id]) as unknown as ScheduleRow;
    ...
    if (schedule.status === 'active') {
      ...
      if (mode === 'per_machine') {
        ...
        await db.run(
          'UPDATE schedules SET next_run_at = ?, agent_next_runs = ? WHERE id = ?',
          [minNextRun(newMap), JSON.stringify(newMap), id]
        );
      } else {
        ...
        await db.run(
          'UPDATE schedules SET next_run_at = ?, agent_next_runs = NULL WHERE id = ?',
...
export async function deleteSchedule(id: string): Promise<void> {
  ...
  await db.run(
    "UPDATE schedules SET status = 'deleted', updated_at = datetime('now') WHERE id = ?",
    [id]
  );
}
...
export async function processSchedules(): Promise<{ processed: number; errors: string[] }> {
  const db = await getDb();
  const errors: string[] = [];
  let processed = 0;
  ...
  for (const row of dueRows) {
    ...
    const activeAgentRows = await db.all(
    ...
    if (mode === 'per_machine' && schedule.agent_next_runs) {
      ...
      await db.run(
        ...
        [minNextRun(map), now.toISOString(), JSON.stringify(map), schedule.id]
      );
      continue;
    }
    ...
    if (schedule.schedule_type === 'once') {
      await db.run(
        "UPDATE schedules SET status = 'completed', last_run_at = ?, updated_at = datetime('now') WHERE id = ?",
      ...
      await db.run(
        "UPDATE schedules SET next_run_at = ?, last_run_at = ?, updated_at = datetime('now') WHERE id = ?",
        [nextRun ? nextRun.toISOString() : null, now.toISOString(), schedule.id]
      );
    }
  }
