...
export class DefenderAnalyticsService {
  ...
  async getDefenseVsSecureScore(days: number): Promise<ScoreComparisonPoint[]> {
    ...
    // Attack simulations only, so it lines up with the rest of the Defender tab.
  ...
  async getTechniqueOverlap(): Promise<TechniqueOverlapItem[]> {
    ...
    // Attack simulations only, so the overlap matches the rest of the Defender tab.
  ...
  async getAlertsForTest(
    ...
    // Primary: delegate to the shared evidence-correlation helper so the
    // drill-down agrees with the enrichment pass by construction. Same
    // query, same hits.
    ...
    // Fallback 2: technique-only (alerts may lack evidence metadata)
