...
agentUpdateRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const latest = await getLatestVersion(agent.os, agent.arch);
    if (!latest) {
      res.status(204).send();
      return;
    }
...
agentUpdateRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    await streamUpdate(os, arch, res);
  })
);
...
export function createAdminUpdateRouter(_buildService: unknown): Router {
  const router = Router();
  ...
  router.get(
    ...
    requirePermission('endpoints:versions:read'),
    asyncHandler(async (_req, res) => {
      res.json({ success: true, data: { available: !!_buildService } });
    })
  );
  ...
  router.delete(
    ...
    asyncHandler(async (req, res) => {
      ...
      const deleted = await deleteVersion(version, os as AgentOS, arch as AgentArch);
      if (!deleted) {
