...
import {favoritesRoute} from "./api/favorites.route.js";
import {
  errorHandler,
  notFoundHandler,
...
async function startServer() {
  ...
  if (repoUrl) {
    ...
    try {
      ...
      console.warn(
        "⚠ Failed to sync test repository:",
        error instanceof Error ? error.message : error,
      );
      console.warn(
        "  Browser module will start without tests. Use /api/browser/tests/sync to retry.",
      );
    ...
    testsSourcePath =
      process.env.TESTS_SOURCE_PATH ||
      path.resolve(__dirname, "../../tests_source");
  ...
  if (repoUrl && process.env.GITHUB_TOKEN) {
    try {
      ...
      console.warn(
        "⚠ GitHub metadata service unavailable:",
        error instanceof Error ? error.message : error,
      );
    }
  }
  ...
  const agentSourcePath =
    process.env.AGENT_SOURCE_PATH || path.resolve(__dirname, "../../agent");
  app.use(
    "/api/agent",
    createAgentRouter({ testsSourcePath, agentSourcePath }),
  );
  ...
  httpServer.listen(PORT, () => {
    ...
    console.log(
      "║   ProjectAchilles Backend Server                          ║",
    );
    console.log(
      "╠═══════════════════════════════════════════════════════════╣",
    );
    ...
    console.log(
      "║     /api/browser/*     - Security Test Browser            ║",
    );
    console.log(
      "║     /api/analytics/*   - Test Results Analytics           ║",
    );
    console.log(
      "║     /api/agent/*       - Achilles Agent Management        ║",
    );    console.log(
      "║     /api/agent/*       - Achilles Agent Management        ║",
    );
        console.log(
      "║     /api/favorites/*   - User Favorites tests             ║",
    );
    console.log(
      "║                                                           ║",
    );
    if (gitSyncService) {
      ...
      console.log(
        `║   Tests: ${status.testCount || 0} tests from ${status.branch} branch             ║`,
      );
    }
    console.log(
      "╚═══════════════════════════════════════════════════════════╝",
