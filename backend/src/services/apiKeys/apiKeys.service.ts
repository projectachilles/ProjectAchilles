...
export function validateApiKey(rawKey: string): AuthenticatedApiKey | null {
  ...
  // SHA-256 (not bcrypt/argon2) is correct here: API keys are 256-bit
  // high-entropy random tokens generated by crypto.randomBytes(32), NOT
  // passwords. The 2^256 keyspace makes brute-force infeasible regardless
  // of hash speed, so the only effect of a slow hash would be added latency
  // on every authenticated request. Same pattern as the CLI refresh tokens
  // in this codebase, and what GitHub PATs / Stripe / AWS access keys use.
