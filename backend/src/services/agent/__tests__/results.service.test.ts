...
describe('results.service', () => {
  ...
  describe('bundle results ingestion', () => {
    ...
    it('throws when bulk reports per-item errors (so the route returns 5xx and the agent retries)', async () => {
      mockEsBulk.mockResolvedValue({
        errors: true,
        items: [
  ...
  describe('ingestion idempotency', () => {
    ...
    it('REGRESSION: distinct tasks ingesting the same bundle produce DISTINCT _ids (no overwrite)', async () => {
      ...
      const sameBundle = makeBundle(); // identical bundle_id, identical control_ids
